...
public class Main {
    ...
    private static void OrdenamientoProductos() throws Exception {
        List<Producto> productos = Factory.getProductoRepository().findAll();
        Factory.getProductoSortService().insertionSort(productos);
        // Factory.getProductoSortService().selectionSort(productos);
        Factory.getCsvService().exportarProductos(productos);
    }
}
